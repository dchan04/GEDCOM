...
$(document).ready(function() {
	...
	$('#addIndivForm').submit(function(e){
	  ...
	  $.ajax({
		 ...
		 type:'get',
		 data: {
			 fileName: document.getElementById("fileList2").options[document.getElementById("fileList2").selectedIndex].value,
			 givenName: $('#givenNameAdd').val(),
			 surname: $('#surnameAdd').val(),
	   },
	   success:function(data){
		   $('#fileLogPanel').html(updateFileLogContents());
		   $('#statusPanel').append('Added a new individual ' + $('#givenNameAdd').val() + ' ' + $('#surnameAdd').val() + ' to file ' + document.getElementById("fileList2").options[document.getElementById("fileList2").selectedIndex].value + '<br>');
		   $('#statusPanel').html(repositionStatusBar());
	   },
	   fail:function(error){
		  $('#statusPanel').append(error);
	...
	$('#getDescendantsForm').submit(function(e){
	  ...
	  $.ajax({
		 ...
		 type:'get',
		 data: {
			 fileName: document.getElementById("fileList3").options[document.getElementById("fileList3").selectedIndex].value,
	...
	$('#getAncestorsForm').submit(function(e){
	  ...
	  $.ajax({
		 ...
		 type:'get',
		 data: {
			 fileName: document.getElementById("fileList4").options[document.getElementById("fileList4").selectedIndex].value,
   ...
   $('#fileList').change(function(e) {
		...
		e.preventDefault();
		  $.ajax({
			 ...
			 type:'get',
			 data: {
				 fileName: document.getElementById("fileList").options[document.getElementById("fileList").selectedIndex].value,
		   },
		   success:function(data){
	...
	document.getElementById('storeAllFiles').onclick = function (e) { 
		...
		for (i = 0; i < document.getElementById("fileLogPanel").childNodes.length; i++) { //loop through all tables in the file log panel
			if (document.getElementById("fileLogPanel").childNodes[i].id != undefined) { //skip undefined (for some reason, theres 1 undefined)
				fNameReplace = document.getElementById("fileLogPanel").childNodes[i].id;
				fileName = fNameReplace.replace(".gedTable", ".ged");
				//console.log(fileName);
				fName = fileName;
				src = document.getElementById("fileLogPanel").childNodes[i].rows[1].cells[1].innerHTML;
				gVersion = document.getElementById("fileLogPanel").childNodes[i].rows[1].cells[2].innerHTML;
				encode = document.getElementById("fileLogPanel").childNodes[i].rows[1].cells[3].innerHTML;
				submitterName = document.getElementById("fileLogPanel").childNodes[i].rows[1].cells[4].innerHTML;
				submitterAddr = document.getElementById("fileLogPanel").childNodes[i].rows[1].cells[5].innerHTML;
				numIndividuals = document.getElementById("fileLogPanel").childNodes[i].rows[1].cells[6].innerHTML;
				numFamilies = document.getElementById("fileLogPanel").childNodes[i].rows[1].cells[7].innerHTML;
				...
				$.ajax({
				   ...
				   error:function(err){
					  console.log("Error: File failed to be stored because you are not logged into a database");
				   }
			   });
			   //get individuals
			   ...
			   $.ajax({
					...
					success:function(data){
						if (Object.keys(data.iList).length == 0) { //no individuals. no need to push any data into INDIVIDUAL table.
		...
		$.ajax({
		   ...
		   success:function(data){
			  $('#statusPanel').append("- Database has " + data.numFiles + " files and " + data.numIndiv + " individuals<br>");
			  $('#statusPanel').html(repositionStatusBar());
		   },
		   error:function(error){
			  $('#statusPanel').append(error);
		...
		e.preventDefault();
		  $.ajax({
		   ...
		   error:function(error){
			  $('#statusPanel').append(error);
			  $('#resultPanel').empty();
			  $('#resultPanel').html("Query 1 Error: You are not logged into a database");
			  $('#statusPanel').html(repositionStatusBar());
		   }
		 });
	};
	...
	document.getElementById('query2').onclick = function (e) { 
		e.preventDefault();
		  $.ajax({
			 ...
			 type:'get',
			 data: {
				 fileName: document.getElementById("fileList5").options[document.getElementById("fileList5").selectedIndex].value,
		   },
		   success:function(data){
			   let obj = data;
			   $('#resultPanel').empty();
		   ...
		   error:function(error){
			  $('#statusPanel').append(error);
			  $('#resultPanel').empty();
			  $('#resultPanel').html("Query 2 Error: You are not logged into a database");
			  $('#statusPanel').html(repositionStatusBar());
		   }
		 });
	};
		...
		e.preventDefault();
		  $.ajax({
		   ...
		   error:function(error){
			  $('#statusPanel').append(error);
			  $('#resultPanel').empty();
			  $('#resultPanel').html("Query 3 Error: Either you are not logged into a database or User input is invalid");
			  $('#statusPanel').html(repositionStatusBar());
		   }
		 });
	};
		...
		e.preventDefault();
		  $.ajax({
		   ...
		   error:function(error){
			  $('#statusPanel').append(error);
			  $('#resultPanel').empty();
			  $('#resultPanel').html("Query 4 Error: Either you are not logged into a database or User input is invalid");
			  $('#statusPanel').html(repositionStatusBar());
		   }
		 });
	};
		...
		e.preventDefault();
		  $.ajax({
		   ...
		   error:function(error){
			  $('#statusPanel').append(error);
			  $('#resultPanel').empty();
			  $('#resultPanel').html("Query 5 Error: Either you are not logged into a database or User input is invalid");
			  $('#statusPanel').html(repositionStatusBar());
		   }
		 });
	};
		...
		e.preventDefault();
		  $.ajax({
		   ...
		   error:function(error){
			  $('#statusPanel').append(error);
			  $('#resultPanel').empty();
			  $('#resultPanel').html("Execute Query Error: Either you are not logged into a database or select statement is Invalid");
			  $('#statusPanel').html(repositionStatusBar());
		   }
		 });
	};
...
function updateFileLogContents() {
	let fName = document.getElementById("fileList2").options[document.getElementById("fileList2").selectedIndex].value.replace("uploads/", "");
	console.log(fName);
	let fileNameId = fName + "Table";
	let currentCount = document.getElementById(fileNameId).rows[1].cells[6].innerHTML;
...
function fileLogInfoCreate() { 
				let table = document.createElement("TABLE");
				table.setAttribute("id", document.getElementById('fileNameCSG').value + "Table");
				let row = table.insertRow(0);
				let cell1 = row.insertCell(0);
				let cell2 = row.insertCell(1);
				let cell3 = row.insertCell(2);
				...
				let fName = "/uploads/" + document.getElementById('fileNameCSG').value;
				cell9.innerHTML = '<a href="' + fName + '">' + document.getElementById('fileNameCSG').value + '</a>';
